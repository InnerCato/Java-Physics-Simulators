...
public class World {
    ...
    public World() {
        ...
        // Generating random float values for position and velocity within the screen bounds
        float mouseX = rad + random.nextFloat() * (ZeroGCollision.screenWidth - 2 * rad);
        float mouseY = rad + random.nextFloat() * (ZeroGCollision.screenHeight - 2 * rad);
    ...
    public void addBall() {
        ...
        if (!isTooClose(mouseX, mouseY)) { 
          Ball ball = null;
            ...
            if (mouseX >= rad && mouseX <= (ZeroGCollision.screenWidth-rad)&& mouseY >= rad && mouseY <= (ZeroGCollision.screenWidth-rad)) 
            {          
    ...
    void removeBallAtCursor(float mouseX, float mouseY, float brushRadius) {
      for (int i = balls.size() - 1; i >= 0; i--) {
          ...
          float distance = (float) Math.sqrt(Math.pow(((mouseX - rad) - ball.getX()),2) + Math.pow(((mouseY - rad) - ball.getY()), 2));
    ...
    public boolean isTooClose(float x, float y) {
        ...
        for (Ball existingBall : balls) {
            float distance = (float) Math.sqrt((existingBall.getX() - x) * (existingBall.getX() - x) + (existingBall.getY() - y) * (existingBall.getY() - y));
